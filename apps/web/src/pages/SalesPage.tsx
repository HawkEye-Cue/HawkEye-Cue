...
const TRADE_CONFIGS: Record<string, TradeConfig> = {
  ...
  'gutter-service': { dealTypes: ['Gutter Cleaning', 'Gutter Installation', 'Gutter Guards', 'Repair', 'Downspout Install', 'Commercial', 'Other'], dealTypeLabel: 'Service Type', valueLabel: 'Job Value' },
  'appliance-repair': { dealTypes: ['Washer', 'Dryer', 'Refrigerator', 'Dishwasher', 'Oven/Stove', 'Microwave', 'Ice Maker', 'Commercial', 'Other'], dealTypeLabel: 'Appliance Type', valueLabel: 'Repair Cost' },
  'chimney-services': { dealTypes: ['Chimney Sweep', 'Chimney Repair', 'Cap Install', 'Liner Install', 'Inspection', 'Fireplace Repair', 'Other'], dealTypeLabel: 'Service Type', valueLabel: 'Job Value' },
  'septic-company': { dealTypes: ['Pumping', 'Repair', 'Installation', 'Inspection', 'Drain Field', 'Tank Replacement', 'Other'], dealTypeLabel: 'Service Type', valueLabel: 'Job Value' },
  'excavation-contractor': { dealTypes: ['Foundation Dig', 'Grading', 'Land Clearing', 'Trenching', 'Demolition', 'Site Prep', 'Other'], dealTypeLabel: 'Project Type', valueLabel: 'Contract Value' },
  'insulation-contractor': { dealTypes: ['Spray Foam', 'Blown-In', 'Batt Install', 'Removal', 'Attic', 'Crawl Space', 'Other'], dealTypeLabel: 'Insulation Type', valueLabel: 'Job Value' },
  'welder': { dealTypes: ['Structural', 'Fabrication', 'Repair', 'Aluminum', 'Stainless', 'Custom', 'Pipe', 'Other'], dealTypeLabel: 'Weld Type', valueLabel: 'Job Value' },
  'cabinet-maker': { dealTypes: ['Kitchen Cabinets', 'Bathroom Vanity', 'Built-Ins', 'Closet System', 'Refacing', 'Custom Furniture', 'Other'], dealTypeLabel: 'Project Type', valueLabel: 'Project Value' },
  'countertop-installer': { dealTypes: ['Granite', 'Quartz', 'Marble', 'Butcher Block', 'Laminate', 'Concrete', 'Solid Surface', 'Other'], dealTypeLabel: 'Material', valueLabel: 'Install Value' },
  'drywall-contractor': { dealTypes: ['New Install', 'Repair', 'Texture', 'Finishing', 'Ceiling', 'Commercial', 'Other'], dealTypeLabel: 'Service Type', valueLabel: 'Job Value' },
  'masonry': { dealTypes: ['Brick', 'Stone', 'Block', 'Concrete', 'Retaining Wall', 'Repair', 'Chimney', 'Other'], dealTypeLabel: 'Material/Type', valueLabel: 'Job Value' },
  'stucco-contractor': { dealTypes: ['New Install', 'Repair', 'Re-Coat', 'EIFS', 'Color Coat', 'Commercial', 'Other'], dealTypeLabel: 'Service Type', valueLabel: 'Job Value' },
  'epoxy-flooring': { dealTypes: ['Garage Floor', 'Commercial Floor', 'Metallic Epoxy', 'Flake System', 'Industrial', 'Repair/Recoat', 'Other'], dealTypeLabel: 'Floor Type', valueLabel: 'Job Value' },
  'commercial-cleaning': { dealTypes: ['Office', 'Retail', 'Medical', 'Industrial', 'Post-Construction', 'Recurring Contract', 'Other'], dealTypeLabel: 'Facility Type', valueLabel: 'Contract Value' },
  'janitorial': { dealTypes: ['Daily Service', 'Weekly Service', 'Deep Clean', 'Floor Care', 'Window Cleaning', 'Event Setup', 'Other'], dealTypeLabel: 'Service Type', valueLabel: 'Monthly Value' },
  'snow-removal': { dealTypes: ['Residential Driveway', 'Commercial Lot', 'Sidewalks', 'Salting/De-icing', 'Roof Removal', 'Seasonal Contract', 'Other'], dealTypeLabel: 'Service Type', valueLabel: 'Contract Value' },
  'arborist': { dealTypes: ['Tree Removal', 'Pruning', 'Stump Grinding', 'Planting', 'Health Assessment', 'Emergency', 'Other'], dealTypeLabel: 'Service Type', valueLabel: 'Job Value' },
  'equipment-rental': { dealTypes: ['Excavator', 'Skid Steer', 'Lift/Scaffold', 'Trailer', 'Compactor', 'Generator', 'Other'], dealTypeLabel: 'Equipment Type', valueLabel: 'Rental Value' },
  'farm-ranch-services': { dealTypes: ['Fencing', 'Barn/Outbuilding', 'Well/Irrigation', 'Land Management', 'Equipment Repair', 'Arena', 'Pasture Seeding', 'Other'], dealTypeLabel: 'Service Type', valueLabel: 'Job Value' },
  'horse-trailer-services': { dealTypes: ['Trailer Repair', 'Custom Build', 'Living Quarters', 'Flooring', 'Electrical', 'Axle/Suspension', 'Annual Service', 'Transport', 'Other'], dealTypeLabel: 'Service Type', valueLabel: 'Job Value' },
  'travel-agent': { dealTypes: ['Vacation Package', 'Cruise', 'Group Trip', 'Honeymoon', 'All-Inclusive', 'Business Travel', 'Adventure Tour', 'Custom Itinerary', 'Other'], dealTypeLabel: 'Trip Type', valueLabel: 'Booking Value' },
};
...
export default function SalesPage() {
  ...
  async function handleAdd() {
    if (!name.trim()) { showToast('⚠️ Enter a name for the sale'); return; }
    setAdding(true);
    try {
      const client = await buildClient();
      const computedValue = policyType === 'Bundle'
        ? bundleItems.reduce((s, i) => s + (parseFloat(i.value) || 0), 0)
        : (parseFloat(value) || 0);
      const result = await client.request<Deal>('POST', '/sales/deals', {
        ...
        folio: (defaultFolioStart && defaultFolioEnd) ? `${defaultFolioStart} to ${defaultFolioEnd}` : '',
      ...
      // If the new deal won't show under the current folio filter (e.g. backdated
      // to a different period), switch to "All Periods" so the user always sees it.
      if (folioFilter === 'current' && !dealInCurrentFolio(result)) {
        setFolioFilter('all');
        showToast('✓ Saved — showing All Periods so you can see it');
      }
      ...
      // 🎉 Confetti on new Won deal (wrapped so it can never break the save flow)
      if (stage === 'won') {
        try {
      ...
      // Surface the real error so users know why a sale didn't log
      const msg = e instanceof Error ? e.message : 'Something went wrong';
      showToast(`❌ Couldn't save: ${msg}`);
      console.error('[sales] Failed to log deal:', e);
  ...
  async function handleUpdateStage(dealId: string, newStage: string) {
    try {
      ...
      console.error('[sales] Failed to update stage:', e);
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Folio History — collapsed by default, compact single-line rows (Pro, folio trades) */}
      {foliosEnabled && isPro && availableFolios.length > 0 && (
        <details className="glass-card">
          <summary className="flex items-center justify-between cursor-pointer font-semibold text-white text-sm">
          ...
          <div className="mt-3 divide-y divide-white/5 max-h-[280px] overflow-y-auto">
            {availableFolios.map((folio) => {
              const wonDeals = deals.filter((d) => d.folio === folio && d.stage === 'won');
      ...
      {/* Linked Accounts + Partner Deals removed from Pipeline — this lives on the
          Summit "Whole Team" view instead, to keep this page focused. */}
