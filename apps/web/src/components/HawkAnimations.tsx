    ...
    <>
      ...
      <div className="fixed left-0 top-0 bottom-0 pointer-events-none z-0 items-center hidden lg:flex">
        <div className="relative w-72 h-full overflow-hidden">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-r from-amber-900/10 to-transparent mix-blend-overlay"></div>
        </div>
      </div>
        ...
        <div className="relative w-72 h-full overflow-hidden">
          <img
            ...
            style={{
              filter: 'brightness(0.3) contrast(1.2) saturate(0.6)',
              transform: 'scaleX(-1)',
              maskImage: 'linear-gradient(to left, rgba(0,0,0,0.7) 0%, rgba(0,0,0,0.3) 50%, transparent 100%)',
              WebkitMaskImage: 'linear-gradient(to left, rgba(0,0,0,0.7) 0%, rgba(0,0,0,0.3) 50%, transparent 100%)',
            }}
          />
          <div className="absolute inset-0 bg-gradient-to-l from-amber-900/10 to-transparent mix-blend-overlay"></div>
        </div>
      </div>

      {/* Ambient glow - hidden on mobile */}
      <div className="fixed left-0 top-1/2 -translate-y-1/2 w-48 h-96 pointer-events-none z-0 bg-amber-700/3 blur-3xl rounded-full hidden lg:block"></div>
      <div className="fixed right-0 top-1/2 -translate-y-1/2 w-48 h-96 pointer-events-none z-0 bg-amber-700/3 blur-3xl rounded-full hidden lg:block"></div>
    </>
  );
}
