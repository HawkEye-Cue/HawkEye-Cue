...
exports.handler = async (event) => {
  try {
    ...
    // PUT /appreciations/{id} — edit appreciation
    const editMatch = path.match(/^\/appreciations\/([^/]+)$/);
    if (method === 'PUT' && editMatch && editMatch[1] !== 'settings') {
      const appreciationId = editMatch[1];
      let body = {};
      try {
        const rawBody = event.isBase64Encoded ? Buffer.from(event.body, 'base64').toString() : event.body;
        body = rawBody ? JSON.parse(rawBody) : {};
      } catch { body = {}; }
      const { taggerName, taggerTrade, platform, postContent, postUrl } = body;
      ...
      if (taggerName !== undefined) { setParts.push('taggerName = :tn'); exprValues[':tn'] = taggerName; }
      if (taggerTrade !== undefined) { setParts.push('taggerTrade = :tt'); exprValues[':tt'] = taggerTrade || null; }
      if (platform !== undefined) { setParts.push('platform = :pl'); exprValues[':pl'] = platform; }
      if (postContent !== undefined) { setParts.push('postContent = :pc'); exprValues[':pc'] = postContent; }
      if (postUrl !== undefined) { setParts.push('postUrl = :pu'); exprValues[':pu'] = postUrl || null; }
      ...
      return ok({ id: appreciationId, updated: true });
    }
